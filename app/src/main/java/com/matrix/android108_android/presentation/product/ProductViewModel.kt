...
class ProductViewModel: ViewModel() {
    ...
    fun loadProductsByCategory(category:String){
        _loading.postValue(true)
        viewModelScope.launch(Dispatchers.IO) {
           try{ val response = productsService.getProductsByCategory(category)
    ...
    fun itemClick(productDto: ProductDto){
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if(productDto.isLiked){
                ...
                val entity = ProductEntity(productDto.id, productDto.title, productDto.price, productDto.thumbnail,true)
                productDao.likeProduct(entity)

            }else{
                val entity = ProductEntity(productDto.id, productDto.title, productDto.price, productDto.thumbnail,false)
